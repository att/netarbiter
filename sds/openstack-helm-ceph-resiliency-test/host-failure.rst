...
Ceph status shows that ceph-mon running on ``voyager3`` becomes out of quorum and ceph-mgr (originally running on ``voyager3``) now runs on ``voyager4``. Six osds running on ``voyager3`` are down (i.e., 18 up out of 24 osds).
